...
--The world total number of cases by date
SELECT date,sum(new_cases)as "World new cases",sum(new_deaths) as "World new deaths",sum(new_deaths)/sum(new_cases)*100
as "Death percentage"
FROM [Portfolio Project]..CovidDeaths$
Where continent is not null
group by date
